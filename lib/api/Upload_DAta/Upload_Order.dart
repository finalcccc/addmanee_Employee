...
Upload_Order(Cartnotifire cart,  ProductNotifire product,EmployeeNotifire emp)async{
  Order_Model_upload Order = Order_Model_upload();
  cart.Carts.forEach((element) async {
    CartModel f = await CartModel(element.Product!.id,element.amout, element.sum);
    Order.Ditell.add(f.toMap());
   await FirebaseFirestore.instance.collection('products').doc(f.Product_ID).update({'amount':FieldValue.increment(-f.amout!)});
  });
